...
const basePlugins = (cssExt, platform, isDEV) => [
  new VueLoaderPlugin(),
  ...
  new FriendlyErrorsPlugin({
    compilationSuccessInfo: {
      ...
      notes: isDEV
        ? []
        : [
            `The compiled files are in directory dist-${platform}  (*^▽^*) Enjoy it~`
          ]
    },
...
// 微信特有 plugin
const wechatPlugins = [copyImages(appMainFile), copyCustomCom]
const createPlugins = (platform, isDEV) => {
  const cssExt = CSS_EXT[platform]
  let plugins = basePlugins(cssExt, platform, isDEV)
  if (platform === 'wechat') {
    plugins.push(...wechatPlugins)
  }
  return plugins
}
...
function createBaseConfig() {
  ...
  const webpackBaseConfig = {
    ...
    plugins: createPlugins(platform, isDEV)
  }
